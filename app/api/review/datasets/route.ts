...
// GET /api/review/datasets        → saved-sheet metadata only (fast listing)
// GET /api/review/datasets?full=1 → every saved sheet WITH its rows (code search)
export async function GET(request: Request) {
  ...
  await connectDB();
  const docs = await ReviewDataset.find(
    { ownerId: userId },
    full ? {} : { rows: 0 },
  )
    .sort({ savedAt: -1 })
...
// POST /api/review/datasets — insert or update one sheet (upsert by client id).
